...
def check_log(line):
    ...
    if not name and email and age in line:
...
with open('files/registrations.txt', 'r', encoding='utf8') as file:
    reg_good = open('files/registrations_good.log', 'w', encoding='utf8')
    reg_bad = open('files/registrations_bad.log', 'w', encoding='utf8')
    for line in file:
        line = line[:-1]
        try:
            check_log(line)
